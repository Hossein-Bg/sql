...
customer_id
,market_date
,ROW_NUMBER()OVER(PARTITION BY customer_id ORDER BY market_date)AS num_of_visit
...
customer_id
,market_date
,DENSE_RANK()OVER(PARTITION BY customer_id ORDER BY market_date)AS num_of_visit
...
SELECT customer_id, market_date, num_of_visit

FROM most_recent_visit

WHERE num_of_visit=1;
...
SELECT 
product_name 
,TRIM(SUBSTR(product_name, INSTR(product_name, '-') + 1)) AS edited_colm
FROM product
